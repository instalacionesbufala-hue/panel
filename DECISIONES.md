...
**Salvaguarda que lleva el panel:**
1. Antes de cualquier POST pregunta `GET ?action=ping` y exige `"panel": true` en la respuesta. Si no está, no envía nada y lo explica.
2. En toda escritura exige que la respuesta repita la acción (`"accion": "panelGuardarConfig"`, etc.). Si no la repite, no da nada por guardado.

**En el backend hace falta** desviar las acciones `panel*` en la primera línea de `doPost` y `doGet` (ver `backend-propuesta/panel-acceso.gs`) y añadir `panel: true` al `ping`.

## 1. Sesión (sección 6 del encargo)

El contrato no fija la forma exacta. Propuesta implementada:

- `POST ?action=panelLogin`, `payload={"clave":"…"}` → `{ ok:true, accion:"panelLogin", token:"…", caduca:"ISO 8601" }`.
...
2. Backend: desvío `panel*` + `ping.panel` + `panelLogin` (propuesta lista en `backend-propuesta/`).
...
5. Probar el panel contra el backend real cuando esté todo lo anterior. Hasta ahora está probado contra un simulador del contrato.
